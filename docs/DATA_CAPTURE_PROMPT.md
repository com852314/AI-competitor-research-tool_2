...
- **只收錄使用者已開好的頁面資料**，不要自行切換分頁、滾動到其他分類、或多抓額外頁面
- 收到資料後，先確認頁面內容與預期相符（例如：要收 TCG NEW 分類，確認頁面顯示的是 NEW 而非 Recommend 或 Popular）
- **若發現頁面內容與預期不符**（分類錯誤、平台不對、資料量明顯異常等）→ 立即回報使用者，說明發現什麼問題，**等使用者確認後再繼續**，不要自行判斷或繼續記錄
- 不確定資料是否正確時，寧可暫停回報，不要猜測
