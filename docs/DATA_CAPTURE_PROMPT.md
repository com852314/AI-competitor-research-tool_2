...
### BAJI New-Old 新遊戲：gameId 從背景圖片 URL 取得

BAJI 新遊戲頁面的 gameId 藏在遊戲卡片的背景圖片 CSS 內，請使用者在 DevTools console 執行：

```javascript
Array.from(document.querySelectorAll('.fav-block__img'))
  .map(el => {
...
**URL 結構：**
```
https://img.b112j.com/upload/game/{廠家代碼}/{幣別}/{gameId}.png
```

例：`CG-SLOT-077`（CG = CREATIVE GAMING）、`YesBingo-SLOT-053`（YesBingo）、`PP-SLOT-722`（Pragmatic Play）

gameId 直接使用 URL 中的檔名（不含 `.png`），不需加前綴。
